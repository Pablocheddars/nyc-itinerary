export default {
  "id": "d6",
  "number": "06",
  "title": "Brooklyn · <em>DUMBO · Williamsburg</em>",
  "date": "Domingo 7 de junio · Salida 9:30",
  "note": "Día completo en Brooklyn. Brunch en Greenpoint (Chez Ma Tante), bajar a DUMBO, Brooklyn Flea (abierto domingos 10am–5pm), Williamsburg, Maison Premiere.",
  "alert": null,
  "phases": [
    {
      "type": "am",
      "label": "Mañana — Greenpoint + DUMBO",
      "events": [
        { "time": "~9:30", "name": "🚕 Uber a Greenpoint (~20 min)", "desc": "Desde el hotel directo a Chez Ma Tante. Greenpoint está pegado a DUMBO — la ruta baja natural hacia el puente.", "addr": null, "tags": [], "tip": null, "isNew": false, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "10:00", "name": "🥞 Chez Ma Tante — brunch Greenpoint", "desc": "Restaurante Michelin en Greenpoint. Los pancakes son legendarios — esponjosos, con mantequilla y maple. Bistro franco-americano con cocina simple y perfecta. Brunch domingos desde las 10am.", "addr": "📍 90 Calyer St, Greenpoint, Brooklyn", "tags": [{ "key": "breakfast", "label": "Brunch" }, { "key": "must", "label": "Must" }], "tip": { "html": "<strong>Reservar en Resy.</strong> Domingo de brunch se llena rápido. Los pancakes son el must, pero el steak tartare y el boudin noir también son espectaculares." }, "isNew": true, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "~11:15", "name": "🍩 Peter Pan Donut & Pastry Shop", "desc": "Ícono de Greenpoint desde 1950s — barra de fórmica, donuts hechos a mano cada mañana. El red velvet y el sour cream glazed son los must. A pasos de Chez Ma Tante.", "addr": "📍 727 Manhattan Ave, Greenpoint · cash", "tags": [{ "key": "bakery", "label": "Bakery" }, { "key": "must", "label": "Must" }], "tip": { "html": "<strong>Longevidad de marca:</strong> 70+ años con producto idéntico, cero rebranding, lealtad de barrio. Un donut clásico compartido." }, "isNew": true, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "~11:30", "name": "🥐 Radio Bakery", "desc": "La bakery más hypeada de Brooklyn (abrió 2023, fila desde temprano). Laminados de masa madre, focaccia, pretzel croissant. El polo opuesto a Peter Pan: artesanal moderno, producto del momento. Llevar algo para el camino.", "addr": "📍 135 India St, Greenpoint", "tags": [{ "key": "bakery", "label": "Bakery benchmark" }, { "key": "must", "label": "Must" }], "tip": { "html": "<strong>Para Brown:</strong> El contraste con Peter Pan en la misma cuadra es la lección — viejo vs nuevo, ambos llenos. Estudiar el laminado, el display minimal y cómo el hype/escasez (se agotan a media mañana) genera fila. Un laminado compartido." }, "isNew": true, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "~11:50", "name": "🚕 Uber a DUMBO (~10 min)", "desc": "Greenpoint a DUMBO es directo por Manhattan Ave.", "addr": null, "tags": [], "tip": null, "isNew": false, "isReplaced": false, "replacedNote": null, "isLogistic": true },
        { "time": "~12:00", "name": "📷 DUMBO — Washington Street", "desc": "La foto entre los dos pilares del puente con Manhattan al fondo. La imagen más compartida de NYC.", "addr": null, "tags": [{ "key": "sight", "label": "Paseo" }], "tip": null, "isNew": false, "isReplaced": false, "replacedNote": null, "isLogistic": false }
      ]
    },
    {
      "type": "pm",
      "label": "Tarde",
      "events": [
        { "time": "12:15", "name": "🍕 Grimaldi's Pizzeria", "desc": "Horno a leña bajo el puente. Margherita con mozzarella fresca. Llegar antes del rush. Walk-in, fila ~20 min.", "addr": null, "tags": [{ "key": "pizza", "label": "Pizza research" }], "tip": { "html": "<strong>Coal-oven benchmark:</strong> Observar el horno, las temperaturas, la velocidad de cocción. Comparar con lo que hará el PizzaMaster eléctrico de Respect. Una margherita compartida." }, "isNew": false, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "13:30", "name": "🧳 Brooklyn Flea Market", "desc": "Vintage, diseño local, food vendors. Para Pablo: conceptos gastronómicos para traer ideas. Abierto sábados y domingos 10am–5pm bajo el Manhattan Bridge Archway.", "addr": "📍 80 Pearl St, DUMBO", "tags": [{ "key": "sight", "label": "Paseo" }, { "key": "must", "label": "Must" }], "tip": null, "isNew": false, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "~14:30", "name": "🍔 Emily — el Emmy Burger original (Clinton Hill)", "desc": "La Emily original (2014, de Matt & Emily Hyland) — acá nació el legendario Emmy Burger: blend dry-aged, queso, cebolla caramelizada y salsa Emmy estilo Big Mac, en pan de pretzel. Top de cualquier lista de mejores burgers de NYC. Uber ~15 min desde DUMBO, de paso a Williamsburg.", "addr": "📍 919 Fulton St, Clinton Hill, Brooklyn", "tags": [{ "key": "burger", "label": "Burger research" }, { "key": "must", "label": "Must" }], "tip": { "html": "<strong>Para Fatty Patty:</strong> El benchmark premium del bar burger. El pan de pretzel y la salsa Emmy son la firma — observar el blend dry-aged, el punto y por qué solo hacen pocas por servicio (escasez intencional). Comparar con El Sazón (Día 3) y Corner Bistro (Día 4). Uno compartido." }, "isNew": true, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "15:15", "name": "🌿 Williamsburg — pasear", "desc": "~10 min en Uber desde Clinton Hill. Murales, tiendas independientes, la escena gastronómica más interesante de NYC fuera de Manhattan.", "addr": null, "tags": [{ "key": "sight", "label": "Paseo" }], "tip": null, "isNew": false, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "~15:30", "name": "☕ Devoción", "desc": "Café colombiano con granos que llegan días después de la cosecha, tostados on-site. Local con pared viva de plantas — como concepto de espacio gastronómico es espectacular.", "addr": "📍 69 Grand St, Williamsburg", "tags": [{ "key": "bakery", "label": "Concepto espacio" }], "tip": { "html": "<strong>Concepto de espacio:</strong> No es por el café — es por cómo diseñaron el local y la experiencia. Relevante para cualquier nuevo concepto de Cheddars Group." }, "isNew": true, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "~16:00", "name": "🍪 Chip City", "desc": "Cadena de cookies premium con sabores rotativos, modelo simple y escalable. Competidor directo de Levain.", "addr": null, "tags": [{ "key": "bakery", "label": "Bakery benchmark" }], "tip": { "html": "<strong>vs Levain:</strong> Comparar tamaño, textura, pricing, branding, rotación de sabores. Una cookie compartida." }, "isNew": true, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "16:30", "name": "🍦 Ample Hills Creamery", "desc": "Helado artesanal Brooklyn. Sabores únicos: Ooey Gooey Butter Cake, The Munchies.", "addr": null, "tags": [], "tip": null, "isNew": false, "isReplaced": false, "replacedNote": null, "isLogistic": false }
      ]
    },
    {
      "type": "night",
      "label": null,
      "nightTitle": "🧪 Maison Premiere — ostras y cócteles Belle Époque",
      "nightSub": "Williamsburg · Top 40 Norteamérica · Reserva confirmada 18:15 (garden). Mañana: Summit One Vanderbilt.",
      "events": [
        { "time": "18:15", "name": "🧪 Maison Premiere — garden (RESERVA)", "desc": "Terraza exterior en junio. Ostras $3–5 c/u + cócteles de absinthe. Uno de los lugares más bellos de NYC. Reservado en garden para las 18:15.", "addr": "✅ Reserva confirmada · Domingo 7, 18:15 · garden", "tags": [{ "key": "bar", "label": "Cena + Bar" }], "tip": null, "isNew": false, "isReplaced": false, "replacedNote": null, "isLogistic": false },
        { "time": "21:00", "name": "🚕 Uber de regreso — UWS (~35–45 min / $35–45)", "desc": "Mañana es Summit One Vanderbilt.", "addr": null, "tags": [], "tip": null, "isNew": false, "isReplaced": false, "replacedNote": null, "isLogistic": false }
      ]
    }
  ]
};
